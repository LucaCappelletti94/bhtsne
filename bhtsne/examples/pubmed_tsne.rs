//! Visualize the PubMed diabetes citation network in 2D with t-SNE under several notions of
//! neighbor, to show how multiple affinity graphs pool into one embedding at a larger scale than the
//! Cora companion example.
//!
//! PubMed is 19717 biomedical abstracts (nodes), each with a 500-dimensional TF-IDF feature vector
//! over a curated Diabetes Mellitus vocabulary and one of 3 topic labels (Experimental, Type 1,
//! Type 2), connected by 44324 citation edges. The dataset ships in `data/pubmed.npz` (numpy
//! compressed archive with `features`, `labels`, and `edges` arrays), so the example runs offline.
//!
//! It renders one `pubmed.png` as a 2x3 panel colored by the 3 topic labels. The top row holds the
...
/// Distinct opaque colors for the three PubMed diabetes topic classes.
...
/// Class names in label-index order (Planetoid PubMed one-hot column order).
const CLASS_NAMES: [&str; N_CLASSES] = [
    "Diabetes_Experimental",
    "Diabetes_Type_1",
    "Diabetes_Type_2",
];
...
fn main() -> Result<(), Box<dyn Error>> {
    let t0 = Instant::now();
    let mut npz = NpzArchive::open("data/pubmed.npz")?;
