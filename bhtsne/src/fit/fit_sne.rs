//! FIt-SNE builder and result (FFT-interpolated repulsion).
...
use crate::repulsion::{InterpolatedRepulsion, Repulsion};
use crate::tsne;

/// FIt-SNE builder (FFT-interpolated repulsion).
///
/// Produced by [`TsneReady::fit_sne`](crate::TsneReady::fit_sne).
pub struct FitSneBuilder<'d, T, U, const D: usize> {
    pub(super) config: FitConfig<'d, T, U, D>,
    pub(super) affinities: Affinities<T>,
}

/// Result of a FIt-SNE fit.
...
impl<'d, T, U, const D: usize> FitSneBuilder<'d, T, U, D>
where
    T: Float
        + Send
        + Sync
        ...
        + FftNum,
    U: Send + Sync,
    barnes_hut_tree::Dim<D>: crate::FftDim,
{
    /// Runs the FIt-SNE optimization loop and returns the fitted result.
    pub fn fit(mut self) -> FittedFitSne<'d, T, U, D> {
        ...
        let mut strategy = InterpolatedRepulsion::<T, D>::new();
