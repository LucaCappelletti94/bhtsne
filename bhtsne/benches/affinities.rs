//! Affinity construction: `Affinities::from_metric` (vantage point tree) vs
//! `Affinities::from_neighbors` (precomputed). Neighbors are built once, outside timing.
mod common;
...
fn bench_affinities(c: &mut Criterion) {
    ...
    for &n in &SIZES {
        ...
        group.bench_with_input(BenchmarkId::new("vptree", n), &n, |b, _| {
            b.iter(|| {
                black_box(Affinities::from_metric(&samples, PERPLEXITY, |a, b| {
                    euclidean(a, b)
                }));
            });
        });
        ...
        group.bench_with_input(BenchmarkId::new("precomputed_neighbors", n), &n, |b, _| {
            b.iter(|| {
                black_box(Affinities::from_neighbors(
                    black_box(&neighbors),
                    PERPLEXITY,
                ));
            });
