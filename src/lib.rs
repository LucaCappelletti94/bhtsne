    ...
    U: Send + Sync,
{
    ...
    // The Send + Sync bounds on the boxed closure preserve the auto traits of the
    // struct, the callback itself is only ever invoked sequentially from the
    // fitting thread.
    #[allow(clippy::type_complexity)]
    epoch_callback: Option<Box<dyn FnMut(usize, &[T]) + Send + Sync + 'data>>,
}
